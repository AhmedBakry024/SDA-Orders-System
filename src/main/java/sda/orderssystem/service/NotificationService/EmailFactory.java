...
public class EmailFactory extends ChannelFactory {
    ...
    public boolean createNotification(Order order) {
            ...
            for (Product product : order.getProducts()) {
                if (randomInt == 0) {
                    notificationQueue.addNotification(
                            new SMSMessage(messageTempelate2(user.getName(), product.getProductName()) + " This is an Email."));
                } else {
                    notificationQueue.addNotification(
                            new SMSMessage(
                                    messageTempelate1(user.getName(), product.getProductName(), order.getStatus()) + " This is an Email."));
                }
        ...
        if (order.getStatus().equals("Shipped")) {
            ...
            for (Product product : order.getProducts()) {
                notificationQueue.addNotification(
                        new SMSMessage(messageTempelate3(user.getName(), product.getProductName(), product.getVendor()) + " This is an Email."));
            }
        }
        return true;
    }
